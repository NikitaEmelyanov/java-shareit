...
public interface BookingRepository extends JpaRepository<Booking, Long> {
    ...
    Collection<Booking> findByBookerIdAndStatus(Long bookerId, STATUS status, Sort sort);
    ...
    Collection<Booking> findByItem_OwnerIdAndStatus(Long ownerId, STATUS status, Sort sort);

    List<Booking> findByItemOwnerIdAndStatusAndItem_Id(Long ownerId, STATUS status, Long itemId);

    List<Booking> findByBookerIdAndItemIdAndEndBeforeAndStatus(
            long bookerId, long itemId, LocalDateTime now, STATUS status);

    Collection<Booking> findAllByItemInAndStatus(Collection<Item> items, STATUS status);
}