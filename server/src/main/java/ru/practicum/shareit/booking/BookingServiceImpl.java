...
public class BookingServiceImpl implements BookingService {
    ...
    public BookingDto getBookingById(Long userId, Long bookingId) {
        ...
        if (!(booking.getUser().getId() == userId
            ...
            throw new UnacceptableValueException(String.format(
                "Current user with %d is not the owner of item and has got no booking of it",
                userId));
        }
    ...
    public BookingDto approveBooking(Long userId, Long bookingId, boolean approved) {
        ...
        if (booking.getState() != BookingState.WAITING) {
            throw new ValidationException(
                String.format("The status of booking should be WAITING. Current status is %s",
                    booking.getState()));
        }
    ...
    public List<BookingDto> getAllBookingsByUser(Long userId, BookingStateFilter state) {
        ...
        switch (state) {
            case BookingStateFilter.CURRENT:
                bookings = bookingRepository.findAllByUserIdAndStartTimeBeforeAndEndTimeAfterOrderByStartTimeDesc(
                    userId,
                    LocalDateTime.now(), LocalDateTime.now());
                break;
            case BookingStateFilter.PAST:
                bookings = bookingRepository.findAllByUserIdAndEndTimeBeforeOrderByStartTimeDesc(
                    userId, LocalDateTime.now());
                break;
            case BookingStateFilter.FUTURE:
                bookings = bookingRepository.findAllByUserIdAndStartTimeAfterOrderByStartTimeDesc(
                    userId, LocalDateTime.now());
                break;
            default:
                if (state == BookingStateFilter.WAITING || state == BookingStateFilter.APPROVED
                    || state == BookingStateFilter.REJECTED) {
                    bookings = bookingRepository.findAllByUserIdAndStateOrderByStartTimeDesc(userId,
    ...
    public List<BookingDto> getAllBookingByOwner(Long ownerId, BookingStateFilter state) {
        ...
        switch (state) {
            case BookingStateFilter.CURRENT:
                bookings = bookingRepository.findCurrentBookingsByOwner(ownerId,
                    LocalDateTime.now());
                break;
            case BookingStateFilter.PAST:
                ...
                break;
            case BookingStateFilter.FUTURE:
                bookings = bookingRepository.findFutureBookingsByOwner(ownerId,
                    LocalDateTime.now());
                break;
            default:
                if (state == BookingStateFilter.WAITING || state == BookingStateFilter.APPROVED
                    || state == BookingStateFilter.REJECTED) {
                    bookings = bookingRepository.findAllByOwnerWithState(ownerId,
                        BookingState.valueOf(state.name()));
                } else {
    ...
    private void validateOwner(Long ownerId, Long userId) {
        if (!Objects.equals(ownerId, userId)) {
            throw new UnacceptableValueException(
                String.format("User with id %d is not the owner of item", userId));
        }
    }
    ...
    private void validatePeriodsOverlap(Item item, LocalDateTime startTime, LocalDateTime endTime) {
        ...
        items.add(item);
        List<Booking> bookings = bookingRepository.findAllCurrentAndFutureBookingForItems(items,
            LocalDateTime.now());
        if (bookings.isEmpty()) {
            return;
        }
        boolean isOverlap = bookings.stream()
            .anyMatch(booking -> (startTime.isAfter(booking.getStartTime()) && startTime.isBefore(
                booking.getEndTime())) ||
                                 (endTime.isAfter(booking.getStartTime()) && endTime.isBefore(
                                     booking.getEndTime())));
        if (isOverlap) {
